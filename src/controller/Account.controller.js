...
const InfoAccount = require('../models/dataAccount.model');
...
class AccountController extends BaseController {
  ...
  async register(req, res) {
    try {
      ...
      const account = new Account({ name, email, password, image_url});
      const newUser = await account.save();
      const dataUser = new InfoAccount({
      ...
      await dataUser.save();
      res.status(201).json({ message: 'Create success', data: dataUser });
    } catch (error) {
      console.error(error);
